...
# Mapping logical layer names to actual table names
layer_table_map = {
    'landuse': 'landuse',
    'roads': 'roads',
    'nepal population density': 'nepal population density',
    'nepal_administrative_boundary': 'nepal_administrative_boundary',
}
...
def get_layer(layer):
    if layer in layer_table_map:
        query = f"""
        SELECT ST_AsGeoJSON(geom), row_to_json(t) FROM {layer_table_map[layer]} as t;
        """
        return jsonify(fetch_geojson(query))
    elif layer in ['locallevel', 'district', 'province']:
        # Placeholder: return empty FeatureCollection for overlays
...
def attribute_query():
    ...
    if layer not in ['landuse', 'nepal population density', 'nepal_administrative_boundary']:
        return jsonify({"error": "Invalid layer"}), 400
    allowed_ops = ['=', '>', '<', '>=', '<=', 'LIKE']
    ...
    FROM {layer_table_map[layer]} as t
    WHERE {attr} {op} %s;
    """
